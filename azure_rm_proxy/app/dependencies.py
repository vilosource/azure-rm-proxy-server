...
def get_azure_service() -> AzureResourceService:
    ...
    try:
        ...
        logger.error(f"Failed to initialize {settings.cache_type} cache, falling back to in-memory cache: {e}")
        cache = CacheFactory.create_cache(CacheType.MEMORY)
    
    limiter = ConcurrencyLimiter(max_concurrent=settings.max_concurrency)
    
    if settings.use_mock:
        logger.info(f"Creating MockAzureResourceService instance with fixtures from {settings.mock_fixtures_dir}")
        return MockAzureResourceService(
            cache=cache, 
            limiter=limiter, 
            fixtures_dir=settings.mock_fixtures_dir
        )
    else:
