    ...
    if vms:
        ...
        if vm_details:
            ...
            if vm_details.network_interfaces:
                logger.info(f"  Network Interfaces: {len(vm_details.network_interfaces)}")
                for i, nic in enumerate(vm_details.network_interfaces):
                    logger.info(f"    NIC {i+1}: {nic.name}")
                    logger.info(f"      Private IPs: {', '.join(nic.private_ip_addresses) if nic.private_ip_addresses else 'None'}")
                    logger.info(f"      Public IPs: {', '.join(nic.public_ip_addresses) if nic.public_ip_addresses else 'None'}")
            
            if vm_details.effective_nsg_rules:
                logger.info(f"  NSG Rules: {len(vm_details.effective_nsg_rules)}")
                for i, rule in enumerate(vm_details.effective_nsg_rules[:3]):  # Show first 3 only
                    logger.info(f"    Rule {i+1}: {rule.name} - {rule.direction} - {rule.protocol} - {rule.port_range} - {rule.access}")
                
                if len(vm_details.effective_nsg_rules) > 3:
                    logger.info(f"    ... and {len(vm_details.effective_nsg_rules) - 3} more NSG rules")
        else:
