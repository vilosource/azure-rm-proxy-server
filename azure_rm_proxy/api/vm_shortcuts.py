...
router = APIRouter(tags=["VM Shortcuts"], prefix="/subscriptions/virtual_machines")
...
async def list_all_virtual_machines(
    ...
    try:
        ...
        for vm in vms:
            vm.detail_url = f"/subscriptions/virtual_machines/{vm.name}"
...
async def get_vm_by_name(
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail=f"Virtual Machine '{vm_name}' not found in any subscription or resource group")
    except ClientAuthenticationError:
        raise HTTPException(status_code=401, detail="Authentication failed")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"An error occurred: {e}")