...
class AzureVNetPeeringService(PeeringServiceInterface):
    ...
    @cached_azure_operation(
        model_class=VirtualNetworkPeeringPairModel, 
        cache_key_prefix="vnet_peering_report"
    )
    ...
    async def _process_vnets_for_peerings(
        ...
        for vnet_raw in vnets:
            try:
                ...
                if not hasattr(vnet_raw, "virtual_network_peerings") or not vnet_raw.virtual_network_peerings:
                    continue
    ...
    def _find_return_peering(self, remote_vnet, local_vnet_id: str) -> Optional[Dict]:
        ...
        """
        if not hasattr(remote_vnet, "virtual_network_peerings") or not remote_vnet.virtual_network_peerings:
            return None
