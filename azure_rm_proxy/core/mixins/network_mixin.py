...
class NetworkMixin(BaseAzureResourceMixin):
    ...
    async def _fetch_network_interfaces(self, vm, network_client, vm_name):
        ...
        """
        network_interfaces = []
        if not (hasattr(vm, 'network_profile') and vm.network_profile and 
                hasattr(vm.network_profile, 'network_interfaces')):
        ...
        for nic_ref in vm.network_profile.network_interfaces:
            ...
            try:
                ...
                public_ips = await self._get_public_ips(nic, network_client, nic_rg_name, vm_name)

                network_interfaces.append(NetworkInterfaceModel(
                    id=nic.id, 
                    name=nic.name, 
    ...
    async def _get_public_ips(self, nic, network_client, nic_rg_name, vm_name):
        ...
        for ip_config in nic.ip_configurations:
            ...
            try:
                if hasattr(ip_config.public_ip_address, 'name') and ip_config.public_ip_address.name:
                    self._log_debug(f"Fetching public IP {ip_config.public_ip_address.name}")
                    public_ip = network_client.public_ip_addresses.get(nic_rg_name, ip_config.public_ip_address.name)
                ...
                self._log_warning(f"Public IP {ip_config.public_ip_address.name if hasattr(ip_config.public_ip_address, 'name') else 'unknown'} not found.")
            except Exception as e:
    ...
    async def _fetch_nsg_rules(self, network_client, resource_group_name, network_interfaces):
        ...
        self._log_debug(f"Fetching effective NSG rules for NIC {first_nic_name} in resource group {nic_rg_name}")
        ...
        try:
            ...
            try:
                ...
                if hasattr(nic, 'network_security_group') and nic.network_security_group:
                    ...
                    if nsg_id:
                        ...
                        if len(nsg_parts) >= 9:
                            ...
                            if nsg and hasattr(nsg, 'security_rules') and nsg.security_rules:
                                for rule in nsg.security_rules:
                                    port_range = rule.destination_port_range or ",".join(rule.destination_port_ranges) if hasattr(rule, 'destination_port_ranges') else "*"
                                    
                                    effective_nsg_rules.append(NsgRuleModel(
                                        name=rule.name,
                ...
                if effective_nsg_rules:
                    self._log_debug(f"Fetched {len(effective_nsg_rules)} NSG rules using direct method")
                    return effective_nsg_rules
            ...
            try:
                ...
                if hasattr(network_client.network_interfaces, 'begin_list_effective_network_security_groups'):
                                ...
                                if hasattr(nsg_result, 'effective_security_rules'):
                                    for rule in nsg_result.effective_security_rules:
                                        ...
                                        effective_nsg_rules.append(NsgRuleModel(
                                            ...
                                            direction=str(rule.direction) if hasattr(rule, 'direction') else "Unknown",
                                            protocol=str(rule.protocol) if hasattr(rule, 'protocol') else "Unknown",
                                            port_range=port_range,
                    ...
                    self._log_debug(f"Fetched {len(effective_nsg_rules)} effective NSG rules using API")
            ...
            self._log_warning(f"Could not fetch effective NSG rules for NIC {first_nic_name}: {str(e)}")
    ...
    async def _fetch_routes(self, network_client, resource_group_name, network_interfaces):
        ...
        self._log_debug(f"Fetching effective routes for NIC {first_nic_name} in resource group {nic_rg_name}")
        ...
        try:
            ...
            try:
                            ...
                            if hasattr(ip_config.subnet, 'id') and ip_config.subnet.id:
                                ...
                                if len(subnet_parts) >= 11:
                                    ...
                                    try:
                                        ...
                                        if hasattr(subnet, 'route_table') and subnet.route_table:
                                            if hasattr(subnet.route_table, 'id') and subnet.route_table.id:
                                                ...
                                                if len(rt_parts) >= 9:
                                                    ...
                                                    if hasattr(rt, 'routes') and rt.routes:
                                                        for route in rt.routes:
                                                            effective_routes.append(RouteModel(
                ...
                if effective_routes:
                    self._log_debug(f"Fetched {len(effective_routes)} routes using direct method")
                    return effective_routes
