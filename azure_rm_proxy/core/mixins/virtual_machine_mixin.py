...
class VirtualMachineMixin(BaseAzureResourceMixin):
    ...
    async def get_vm_details(
        ...
        self._log_info(
            f"Successfully fetched VM details for {vm_name}"
        )
        return vm_detail
    ...
    async def _fetch_network_interfaces(self, vm, network_client, vm_name):
        ...
        network_interfaces = []
        if not hasattr(vm, 'network_profile') or not vm.network_profile or not vm.network_profile.network_interfaces:
        ...
        for nic_ref in vm.network_profile.network_interfaces:
            try:
                ...
                self._log_debug(f"Fetching details for NIC {nic_name} in resource group {nic_resource_group}")
    ...
    async def _fetch_aad_groups(self, subscription_id, vm):
        ...
        try:
            ...
            if not vm.identity.principal_id and (not hasattr(vm.identity, "user_assigned_identities") or not vm.identity.user_assigned_identities):
                return aad_groups
            ...
            if vm.identity.principal_id:
                self._log_debug(f"Fetching AAD groups for system assigned identity {vm.identity.principal_id}")
                ...
                for assignment in auth_client.role_assignments.list_for_scope(vm.id):
                    if hasattr(assignment, "principal_id") and assignment.principal_id == vm.identity.principal_id:
            ...
            if hasattr(vm.identity, "user_assigned_identities") and vm.identity.user_assigned_identities:
                for identity_id, identity in vm.identity.user_assigned_identities.items():
                    ...
                    try:
                        ...
                        self._log_warning(f"Error fetching AAD groups for identity {identity_id}: {e}")
