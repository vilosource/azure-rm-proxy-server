...
from ..models import VirtualMachineModel, VirtualMachineDetail, VirtualMachineWithContext, VirtualMachineHostname, SubscriptionModel
...
class VirtualMachineMixin(BaseAzureResourceMixin):
    ...
    async def get_virtual_machines(self, subscription_id: str, resource_group_name: str, refresh_cache: bool = False) -> List[VirtualMachineModel]:
        ...
        self._log_debug(f"Attempting to get VMs for RG {resource_group_name} in subscription {subscription_id} with refresh_cache={refresh_cache}")
        
        if not refresh_cache:
            cached_data = self.cache.get(cache_key)
            if cached_data:
                self._log_debug(f"Cache hit for VMs in resource group {resource_group_name} in subscription {subscription_id}")
                return cached_data

        self._log_info(f"Fetching VMs for resource group {resource_group_name} in subscription {subscription_id} from Azure")
        try:
            compute_client = AzureClientFactory.create_compute_client(subscription_id, self.credential)
            
            async with self.limiter:
                self._log_debug(f"Acquired concurrency limiter for VMs in RG {resource_group_name} in subscription {subscription_id}")
                ...
                self._log_debug(f"Released concurrency limiter for VMs in RG {resource_group_name} in subscription {subscription_id}")
                
            self._set_cache_with_ttl(cache_key, virtual_machines, settings.cache_ttl)
            self._log_info(f"Fetched {len(virtual_machines)} VMs for resource group {resource_group_name} in subscription {subscription_id}")
            return virtual_machines
            
        except ResourceNotFoundError as e:
            self._log_warning(f"Resource group {resource_group_name} not found in subscription {subscription_id}: {e}")
            raise
        except ClientAuthenticationError as e:
            self._log_error(f"Authentication error fetching VMs: {e}")
            raise
        except Exception as e:
            self._log_error(f"Error fetching VMs for resource group {resource_group_name} in subscription {subscription_id}: {e}")
            raise
    ...
    def _create_vm_model_from_azure_vm(self, vm):
        ...
        vm_dict = {
            ...
            "name": vm.name,
            "location": vm.location,
            "vm_size": vm.hardware_profile.vm_size if hasattr(vm, 'hardware_profile') and vm.hardware_profile else "Unknown",
            "os_type": vm.storage_profile.os_disk.os_type if hasattr(vm, 'storage_profile') and vm.storage_profile and 
                    hasattr(vm.storage_profile, 'os_disk') and vm.storage_profile.os_disk else None,
            "power_state": None
        }
        return VirtualMachineModel.model_validate(vm_dict)
    
    async def get_vm_details(self, subscription_id: str, resource_group_name: str, vm_name: str, refresh_cache: bool = False) -> VirtualMachineDetail:
        """
        ...
        """
        cache_key = self._get_cache_key(["vm_details", subscription_id, resource_group_name, vm_name])
        self._log_debug(f"Attempting to get VM details for {vm_name} with refresh_cache={refresh_cache}")
        ...
        self._log_info(f"Fetching VM details for {vm_name} in RG {resource_group_name} in subscription {subscription_id} from Azure")
        
        try:
            async with self.limiter:
                self._log_debug(f"Acquired concurrency limiter for VM details of {vm_name}")
                
                compute_client = AzureClientFactory.create_compute_client(subscription_id, self.credential)
                network_client = AzureClientFactory.create_network_client(subscription_id, self.credential)
                ...
                self._log_info(f"Successfully fetched and cached VM details for {vm_name}")
                self._log_debug(f"Released concurrency limiter for VM details of {vm_name}")
                return vm_detail

        except ResourceNotFoundError as e:
            self._log_warning(f"VM {vm_name} not found in RG {resource_group_name} in subscription {subscription_id}: {e}")
            raise
        except ClientAuthenticationError as e:
    ...
    async def get_all_virtual_machines(self, refresh_cache: bool = False) -> List[VirtualMachineWithContext]:
        """
        ...
        try:
            ...
            for sub in subscriptions:
                try:
                    resource_groups = await self.get_resource_groups(sub.id, refresh_cache=refresh_cache)
                    
                    for rg in resource_groups:
                        try:
                            vms = await self.get_virtual_machines(sub.id, rg.name, refresh_cache=refresh_cache)
                            for vm in vms:
                            ...
                            self._log_warning(f"Error fetching VMs for resource group {rg.name} in subscription {sub.id}: {e}")
                            continue
                except Exception as e:
    ...
    async def find_vm_by_name(self, vm_name: str, refresh_cache: bool = False) -> VirtualMachineDetail:
        """
        ...
        self._log_debug(f"Attempting to find VM {vm_name} with refresh_cache={refresh_cache}")
                        ...
                        try:
                            ...
                            for vm in vms:
                                if vm.name.lower() == vm_name.lower():
                                    vm_details = await self.get_vm_details(sub.id, rg.name, vm.name, refresh_cache=refresh_cache)
                                    self._set_cache_with_ttl(cache_key, vm_details, settings.cache_ttl)
                                    return vm_details
                        except Exception as e:
                            self._log_warning(f"Error fetching VMs for resource group {rg.name} in subscription {sub.id}: {e}")
                            continue
    ...
    async def get_vm_hostnames(self, subscription_id: str = None, refresh_cache: bool = False) -> List[VirtualMachineHostname]:
        """
        ...
        self._log_debug(f"Attempting to get VM hostnames with refresh_cache={refresh_cache}")
        ...
        self._log_info(f"Fetching VM hostnames for subscription {'all' if subscription_id is None else subscription_id}")
        ...
        try:
            ...
            for sub in subscriptions:
                ...
                try:
                    resource_groups = await self.get_resource_groups(sub.id, refresh_cache=refresh_cache)
                    self._log_info(f"Found {len(resource_groups)} resource groups in subscription {sub.id}")
                    
                    for rg in resource_groups:
                        try:
                            self._log_debug(f"Processing resource group: {rg.name} in subscription {sub.id}")
                            compute_client = AzureClientFactory.create_compute_client(sub.id, self.credential)
                            ...
                            self._log_warning(f"Error fetching VMs for resource group {rg.name} in subscription {sub.id}: {str(e)}")
                            continue
                except Exception as e:
                    self._log_warning(f"Error fetching resource groups for subscription {sub.id}: {str(e)}")
                    continue
            
            if not vm_hostnames and subscription_id is None:
                self._log_warning("No VM hostnames found across any subscriptions. This might indicate an issue with permissions or connectivity.")
                    
            self._set_cache_with_ttl(cache_key, vm_hostnames, settings.cache_ttl)
            self._log_info(f"Fetched hostnames for {len(vm_hostnames)} VMs across {'all subscriptions' if subscription_id is None else 'subscription ' + subscription_id}")
            return vm_hostnames
        except Exception as e:
