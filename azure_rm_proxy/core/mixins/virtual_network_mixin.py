...
from ..models import VirtualNetworkModel, SubnetModel, VirtualNetworkPeeringModel, VirtualNetworkPeeringPairModel
...
class VirtualNetworkMixin(BaseAzureResourceMixin):
    ...
    @cached_azure_operation(
        model_class=VirtualNetworkModel, cache_key_prefix="virtual_networks"
    )
    async def list_virtual_networks(
        self, subscription_id: str, resource_group_name: Optional[str] = None, refresh_cache: bool = False
    ...
    async def get_virtual_network(
        self, subscription_id: str, resource_group_name: str, vnet_name: str, refresh_cache: bool = False
                ...
                for endpoint in subnet.service_endpoints:
                    if hasattr(endpoint, "service") and endpoint.service:
                        endpoint_model = {
                            ...
                            "locations": [],
                            "provisioning_state": endpoint.provisioning_state if hasattr(endpoint, "provisioning_state") else None
                        }
                        ...
                        service_endpoints.append(ServiceEndpointModel.model_validate(endpoint_model))
    ...
    async def _fetch_peerings(
        self, vnet: VirtualNetwork, network_client: NetworkManagementClient, resource_group_name: str
        ...
        for peering in vnet.virtual_network_peerings:
            remote_vnet_id = ""
            if (hasattr(peering, "remote_virtual_network") and 
                peering.remote_virtual_network and
                hasattr(peering.remote_virtual_network, "id")):
    ...
    async def get_peering_report(
        ...
        try:
            ...
            self._log_debug(f"Found {len(all_vnets_raw)} virtual networks in subscription {subscription_id}")
            ...
            for vnet_raw in all_vnets_raw:
                try:
                    if not hasattr(vnet_raw, "id") or not hasattr(vnet_raw, "name") or not vnet_raw.id or not vnet_raw.name:
                        continue
                    ...
                    vnet_resource_group = self._extract_resource_group_from_id(vnet_id, resource_group_name)
                    
                    self._log_debug(f"Processing VNet: {vnet_name} in resource group {vnet_resource_group}")
                    
                    # Get peerings directly rather than through the detailed VNet model
                    # This avoids issues with subnet validation errors
                    if hasattr(vnet_raw, "virtual_network_peerings") and vnet_raw.virtual_network_peerings:
                        ...
                        for peering in peerings:
                            try:
                                ...
                                self._log_debug(f"Processing peering pair: {pair_id} between {vnet_name} and remote VNet")
                                ...
                                peering_data = {
                                    ...
                                    "name": peering.name,
                                    "remote_virtual_network_id": remote_vnet_id,
                                    "allow_virtual_network_access": peering.allow_virtual_network_access if hasattr(peering, "allow_virtual_network_access") else True,
                                    "allow_forwarded_traffic": peering.allow_forwarded_traffic if hasattr(peering, "allow_forwarded_traffic") else False,
                                    "allow_gateway_transit": peering.allow_gateway_transit if hasattr(peering, "allow_gateway_transit") else False,
                                    "use_remote_gateways": peering.use_remote_gateways if hasattr(peering, "use_remote_gateways") else False,
                                    "peering_state": peering.peering_state if hasattr(peering, "peering_state") else "Unknown",
                                    "provisioning_state": peering.provisioning_state if hasattr(peering, "provisioning_state") else "Unknown"
                                }
                                
                                # Try to get info on remote VNet
                                remote_vnet_info = await self._get_vnet_info_from_id(remote_vnet_id, refresh_cache)
                                ...
                                try:
                                    ...
                                    remote_vnet_raw = remote_network_client.virtual_networks.get(remote_resource_group, remote_vnet_name)
                                    ...
                                    if hasattr(remote_vnet_raw, "virtual_network_peerings") and remote_vnet_raw.virtual_network_peerings:
                                        for remote_peering in remote_vnet_raw.virtual_network_peerings:
                                            remote_peering_target_id = None
                                            if (hasattr(remote_peering, "remote_virtual_network") and 
                                                remote_peering.remote_virtual_network and
                                                hasattr(remote_peering.remote_virtual_network, "id")):
                                                remote_peering_target_id = remote_peering.remote_virtual_network.id
                                            
                                            if remote_peering_target_id == vnet_id:
                                                # Found the return peering
                                                return_peering_data = {
                                                    "id": remote_peering.id if hasattr(remote_peering, "id") else "",
                                                    "name": remote_peering.name if hasattr(remote_peering, "name") else "",
                                                    "remote_virtual_network_id": vnet_id,
                                                    "peering_state": remote_peering.peering_state if hasattr(remote_peering, "peering_state") else "Unknown",
                                                }
                                                break
                                    ...
                                    self._log_error(f"Error accessing remote VNet {remote_vnet_name}: {str(e)}")
                                    peering_pair = self._create_partial_peering_pair_directly(
                                        vnet_data, 
                                        peering_data, 
                                        remote_vnet_id
                                ...
                                self._log_error(f"Error processing peering {getattr(peering, 'name', 'unknown')}: {str(e)}")
                except Exception as e:
                    self._log_error(f"Error processing VNet {getattr(vnet_raw, 'name', 'unknown')}: {str(e)}")
    ...
    def _create_peering_pair_directly(
        ...
        if peering2to1_data:
            connected = (
                peering1to2_data["peering_state"] == "Connected" and
                peering2to1_data["peering_state"] == "Connected"
            )
        ...
        return VirtualNetworkPeeringPairModel(
            ...
            vnet2_to_vnet1_state=peering2to1_data["peering_state"] if peering2to1_data else "NotConfigured",
