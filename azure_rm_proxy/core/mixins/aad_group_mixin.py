...
class AADGroupMixin(BaseAzureResourceMixin):
    ...
    async def _fetch_aad_groups(self, subscription_id, vm):
        ...
        try:
            ...
            authorization_client = AzureClientFactory.create_authorization_client(subscription_id, self.credential)
            role_assignments = []
            ...
            for ra in authorization_client.role_assignments.list_for_scope(scope=vm.id):
                if hasattr(ra, 'principal_type') and ra.principal_type == "Group" and hasattr(ra, 'principal_id'):
            ...
            for ra in role_assignments:
                    ...
                    if hasattr(ra, 'role_definition_id') and ra.role_definition_id:
                        ...
                        try:
                            ...
                            if len(parts) >= 6:
                                ...
                                role_def = authorization_client.role_definitions.get_by_id(role_def_id)
                                role_name = role_def.role_name if hasattr(role_def, 'role_name') else "Unknown Role"
                                
                                # Create a meaningful display name with the role
                                display_name = f"{role_name} Group ({group_id})"
                                aad_groups.append(AADGroupModel(id=group_id, display_name=display_name))
                                continue
                        except Exception as role_error:
                            self._log_warning(f"Error getting role definition for {role_def_id}: {str(role_error)}")
                ...
                if not any(g.id == group_id for g in aad_groups):
                    aad_groups.append(AADGroupModel(id=group_id, display_name=f"Group {group_id}"))
            ...
            try:
                ...
                for group in aad_groups:
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            updated_groups.append(AADGroupModel(id=group.id, display_name=full_name))
                            self._log_debug(f"Enhanced group name with Graph API: {full_name}")
                        else:
                            updated_groups.append(group)
                    except Exception as graph_error:
                        self._log_warning(f"Could not fetch group details from Graph API for {group.id}: {str(graph_error)}")
                        updated_groups.append(group)
                ...
                self._log_warning("Microsoft Graph SDK not available, using role-based group information")
            except Exception as e:
